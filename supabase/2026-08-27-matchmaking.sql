...
begin
  ...
  -- 2. No joinable 'waiting' game — create one, caller is player 1. Omit
  --    board_layout when the caller didn't supply one so the column DEFAULT
  --    (generate_board_layout(), from 2026-08-27-board-layout.sql) applies —
  --    an explicit NULL would violate the NOT NULL constraint once that file
  --    has been run. Falls back to a NULL layout (self-healed client-side by
  --    GameScreen.load()) if board-layout.sql hasn't run yet.
